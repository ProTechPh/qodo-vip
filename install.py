#!/usr/bin/env python3
"""
Installer script for Qodo VIP

This script sets up a virtual environment and installs the Qodo VIP package.
...
def install_package(venv_path, package_path="."):
    ...
    info(f"Installing Qodo VIP package...")

    try:
        # Install the package in development mode
        subprocess.check_call([str(pip_path), "install", "-e", package_path])
        success("Qodo VIP package installed successfully")
        return True
...
def run_command(venv_path, command_args):
    ...
    if platform.system() == "Windows":
        script_path = venv_path / "Scripts" / "qodo-vip.exe"
    else:
        script_path = venv_path / "bin" / "qodo-vip"

    if not script_path.exists():
        error(f"Command not found: qodo-vip")
        return False
    ...
    try:
        subprocess.check_call([str(script_path)] + args)
        return True
    except subprocess.CalledProcessError as e:
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Install Qodo VIP")
    ...
    info("Starting installation process for Qodo VIP")
    ...
    if args.clean or args.all:
        ...
        run_command(venv_path, ["clean"])

    if args.modify_ids or args.all:
        info("Running telemetry ID modification...")
        run_command(venv_path, ["modify-ids"])

    # Print usage information
    if platform.system() == "Windows":
        cmd_path = f"{venv_path}\\Scripts\\qodo-vip"
    else:
        cmd_path = f"{venv_path}/bin/qodo-vip"

    info("You can now use Qodo VIP with the following commands:")
