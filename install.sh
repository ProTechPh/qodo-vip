...
# install.sh
#
# Description: Installation script for the Qodo VIP project (Python version)
...
REPO_URL="https://raw.githubusercontent.com/ProTechPh/Qodo-vip/main"
...
download_python_installer() {
    ...
    PROJECT_ROOT="$SCRIPT_DIR/Qodo-vip"
    ...
    mkdir -p "$PROJECT_ROOT/qodo_vip"

    # List of Python files to download
    PYTHON_FILES=(
        "qodo_vip/__init__.py"
        "qodo_vip/utils.py"
        "qodo_vip/db_cleaner.py"
        "qodo_vip/id_modifier.py"
        "qodo_vip/cli.py"
        "setup.py"
        "requirements.txt"
    )
...
show_help() {
    echo "Qodo VIP Installation Script (Python Version)"
    echo
...
main() {
    ...
    log_info "Starting installation process for Qodo VIP (Python Version)"
    ...
    # Get the path to the qodo-vip command
    if [ "$PYTHON_CMD" = "python3" ]; then
        qodo_CMD="$PROJECT_ROOT/.venv/bin/qodo-vip"
    else
        if [[ "$OSTYPE" == "msys"* || "$OSTYPE" == "cygwin"* ]]; then
            qodo_CMD="$PROJECT_ROOT/.venv/Scripts/qodo-vip.exe"
        else
            qodo_CMD="$PROJECT_ROOT/.venv/bin/qodo-vip"
        fi
    fi
    ...
    log_info "You can now use QODO VIP with the following commands:"
    log_info "  $QODO_CMD clean       - Clean VS Code databases"
    log_info "  $QODO_CMD modify-ids  - Modify telemetry IDs"
    log_info "  $QODO_CMD all         - Run all tools"
}
