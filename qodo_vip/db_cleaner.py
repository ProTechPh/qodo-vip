...
def clean_vscode_db() -> bool:
    """
    Clean VS Code databases by removing entries containing "qodo"
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM ItemTable WHERE key LIKE '%codium%'")
        count_before = cursor.fetchone()[0]
        
        if count_before == 0:
            info("No qodo-related entries found in the database")
            conn.close()
            return True
        
        # Delete records containing "qodo"
        cursor.execute("DELETE FROM ItemTable WHERE key LIKE '%codium%'")
        conn.commit()
        ...
        success(f"Removed {count_before - count_after} Qodo-related entries from the database")
        return True
